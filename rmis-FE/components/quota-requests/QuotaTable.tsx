// RMIS-FE/components/quota-requests/QuotaTable.tsx
...
//   return (
//     <span
//       className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${s.badge}`}
//     >
...
// rmis27
    data: QuotaRequest[];
    isLoading: boolean;
    onReview: (id: string, requestId: string) => void; // ← new prop
}
...
export default function QuotaTable({
  ...
  const actionStyle = (status: QuotaStatus) => {
    ...
    <>
      ...
      {toast && (
        <div className="fixed top-6 right-6 z-50 flex items-center gap-2 bg-gray-900 text-white text-sm font-semibold px-5 py-3 rounded-xl shadow-xl">
          ...
          {toast}
// rmis 27
export default function QuotaTable({ data, isLoading, onReview }: QuotaTableProps) {
    ...
    return (
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    ...
                    {!isLoading && data.map((row) => (
                        ...
                        >
                            ...
                            <td className="px-6 py-4 text-right">
                                {/* ← calls onReview with UUID and display ID */}
                                <button
                                    onClick={(e) => {
                                        e.stopPropagation();
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    <button
                      ...
                      className={`font-semibold text-sm transition-colors ${actionStyle(row.status)}`}
                    >
      ...
      {logTarget && (
        <ViewLogModal request={logTarget} onClose={() => setLogTarget(null)} />
      )}
    </>
  );
}
