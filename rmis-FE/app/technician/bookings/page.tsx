...
function statusMeta(status: string) {
  switch (status) {
    ...
    default:
      return { label: status, color: '#475569', bg: '#f8fafc', border: '#e2e8f0' };
  }
}
...
export default function TechnicianBookingsPage() {
  ...
  const fetchByStatus = async (status: string) => {
    setActiveTab(status);
    if (status === 'ALL') { fetchBookings(); return; }
    setIsLoading(true);
    setError('');
    try {
      const res = await authFetch(`${API_BASE}/technician/bookings?status=${status}`);
      if (!res.ok) throw new Error();
      setBookings(await res.json());
    } catch {
      ...
      <div style={s.page}>
        <div style={s.centerWrap}>
          <div style={s.resultCard}>
            ...
            <button style={s.btnPrimary} onClick={() => router.push('/')}>← Go Home</button>
          </div>
        </div>
        <style>{spin}</style>
      </div>
        ...
        <div style={s.headerInner}>
          <div style={s.headerLeft}>
            ...
            <div style={s.breadcrumb}>
              ...
              <Link href="/technician/dashboard" style={s.breadcrumbLink}>Dashboard</Link>
              <span style={s.breadcrumbSep}>•</span>
              <Link href="/technician/availability" style={s.breadcrumbLink}>Availability</Link>
              <span style={s.breadcrumbSep}>•</span>
      ...
      <main style={s.main}>
        ...
        <section style={s.hero}>
          ...
          <div style={s.heroContent}>
            ...
            <p style={s.heroSub}>
              View all service tickets assigned to you. Track status, review customer
              details, and stay on top of your upcoming jobs.
            </p>
          </div>
          {/* ── Stat tiles ── */}
              ...
              return (
                <article key={booking.id} style={s.card}>
                  ...
                  <div style={s.cardBody}>
                    <InfoRow icon="👤" label="Customer" value={booking.customerName} />
                    <InfoRow icon="📅" label="Date" value={formatDate(booking.scheduledDate)} />
                    <InfoRow
                      icon="🕐"
                      label="Time"
                  ...
                  <div style={s.cardFooter}>
                    <span style={s.createdAt}>
                      Raised {new Date(booking.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </span>
                    <button
                      style={s.detailBtn}
                      onClick={() => router.push(`/technician/bookings/${booking.id}`)}
                    >
                      View Details →
                    </button>
                  </div>
...
function HeroStat({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.08em', marginTop: 4 }}>{label}</div>
    </div>
  );
}
...
const s: Record<string, CSSProperties> = {
  ...
  headerInner: { maxWidth: 1200, margin: '0 auto', padding: '14px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
  ...
  hero: {
    ...
    padding: '40px 40px 32px', marginBottom: 28,
    display: 'flex', flexDirection: 'column', gap: 28,
  },
  heroOverlay: {
    position: 'absolute', inset: 0, pointerEvents: 'none',
    background: 'radial-gradient(ellipse 70% 80% at 80% 50%, rgba(255,255,255,0.04), transparent)',
  },
  ...
  heroBadge: {
    ...
    color: '#a7f3d0', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 14,
  },
  heroBadgeDot: { width: 6, height: 6, borderRadius: '50%', background: '#34d399', display: 'inline-block' },
  heroTitle: { fontSize: 36, fontWeight: 800, color: '#fff', margin: '0 0 10px', lineHeight: 1.15 },
  heroSub: { fontSize: 14, color: 'rgba(255,255,255,0.65)', lineHeight: 1.7, margin: 0 },
  heroStats: { position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' },
  ...
  tabBtnActive: { background: '#ecfdf5', border: '1px solid #a7f3d0', color: '#047857' },
  tabCount: { borderRadius: 999, padding: '1px 8px', fontSize: 11, fontWeight: 700 },

  // loading / error / empty
  loadingWrap: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 0', gap: 16 },
  ...
  errorBanner: { background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '14px 20px', fontSize: 13, color: '#dc2626', fontWeight: 600 },
  emptyWrap: { textAlign: 'center', padding: '72px 20px' },
  emptyIcon: { fontSize: 40, marginBottom: 16 },
  emptyTitle: { fontSize: 20, fontWeight: 700, color: '#1e293b', margin: '0 0 8px' },
  ...
  ticketNum: { fontSize: 13, fontWeight: 700, color: '#a7f3d0', fontFamily: 'monospace', marginBottom: 4 },
  serviceType: { fontSize: 16, fontWeight: 700, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  statusBadge: { fontSize: 11, fontWeight: 700, borderRadius: 999, padding: '3px 10px', letterSpacing: '0.06em', textTransform: 'uppercase', whiteSpace: 'nowrap', flexShrink: 0 },

  cardBody: { padding: '16px 20px', flex: 1, display: 'flex', flexDirection: 'column', gap: 10 },
  infoRow: { display: 'flex', alignItems: 'center', gap: 10 },
  infoIcon: { width: 32, height: 32, borderRadius: 8, background: '#f0fdf4', border: '1px solid #d1fae5', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, flexShrink: 0 },
  infoLabel: { fontSize: 10, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 1 },
  infoVal: { fontSize: 13, color: '#1e293b', fontWeight: 600 },

  cardFooter: { padding: '12px 20px', borderTop: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
  ...
  resultCard: { background: '#fff', border: '1px solid #e2e8f0', borderRadius: 20, padding: '36px 32px', textAlign: 'center', maxWidth: 380, width: '100%', boxShadow: '0 8px 32px rgba(0,0,0,0.08)' },
  ...
  footer: { textAlign: 'center', padding: '24px 32px', borderTop: '1px solid #f1f5f9', fontSize: 12, color: '#cbd5e1' },
};