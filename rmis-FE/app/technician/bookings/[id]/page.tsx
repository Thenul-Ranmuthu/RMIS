...
function getActions(status: string): { label: string; nextStatus: string; style: 'primary' | 'danger' }[] {
  switch (status) {
    case 'PENDING':
      return [
...
export default function BookingDetailPage() {
  ...
  const handleStatusUpdate = async (nextStatus: string) => {
    if (nextStatus === 'CANCELLED') { setShowCancelModal(true); return; }
    setUpdating(true);
    setUpdateError('');
    try {
      const res = await authFetch(`${API_BASE}/technician/bookings/${id}/status`, {
        method: 'PATCH',
  ...
  const handleCancelConfirm = async () => {
    if (!cancelReason.trim()) { setCancelError('Please provide a cancellation reason.'); return; }
    setUpdating(true);
    setCancelError('');
    try {
      const res = await authFetch(`${API_BASE}/technician/bookings/${id}/status`, {
  ...
  return (
    <div style={s.page}>
      ...
      <main style={s.main}>
        <button style={s.backBtn} onClick={() => router.push('/technician/bookings')}>← Back to Bookings</button>
        ...
        <section style={s.hero}>
          ...
          <div style={s.heroRight}>
            <div style={s.heroStatusBox}>
              ...
              <div style={{ ...s.statusBadgeLg, color: st.color, background: st.bg, border: `1px solid ${st.border}` }}>
                {st.label}
              </div>
            </div>
          </div>
        ...
        {actions.length > 0 && (
          <div style={s.actionBar}>
            <div>
              ...
              <div style={s.actionBarSub}>Move this booking to the next stage</div>
            </div>
            <div style={s.actionBtns}>
              {actions.map(action => (
                <button
                  key={action.nextStatus}
                  style={action.style === 'primary' ? s.actionBtnPrimary : s.actionBtnDanger}
        ...
        {booking.status === 'CANCELLED' && booking.cancellationReason && (
          <div style={s.cancelledBanner}>
            ...
            <div style={s.cancelledBannerReason}>Reason: {booking.cancellationReason}</div>
          </div>
        )}
        ...
        <div style={s.detailGrid}>
          ...
          <div style={{ ...s.detailCard, ...s.detailCardFull }}>
            ...
            <div style={s.contactActions}>
              <a href={`mailto:${booking.customerEmail}`} style={s.contactBtn}>✉ Send Email</a>
...
function DetailRow({
  label, value, highlight, muted, isLink,
}: {
  label: string; value: string; highlight?: boolean; muted?: boolean; isLink?: string;
}) {
  return (
...
const s: Record<string, CSSProperties> = {
  ...
  hero: { position: 'relative', borderRadius: 24, overflow: 'hidden', background: 'linear-gradient(135deg, #064e3b 0%, #065f46 55%, #047857 100%)', padding: '36px 40px', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 24 },
  ...
  heroBadge: { display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 999, padding: '4px 14px', fontSize: 11, fontWeight: 700, color: '#a7f3d0', textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginBottom: 14 },
  ...
  heroStatusBox: { background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.18)', borderRadius: 16, padding: '20px 28px', textAlign: 'center' as const, backdropFilter: 'blur(8px)' },
  ...
  errorBanner: { background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '12px 18px', fontSize: 13, color: '#dc2626', fontWeight: 600, marginBottom: 16 },

  actionBar: { background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: '20px 24px', marginBottom: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.04)' },
  actionBarTitle: { fontSize: 15, fontWeight: 700, color: '#0f172a', marginBottom: 3 },
  actionBarSub: { fontSize: 12, color: '#94a3b8' },
  actionBtns: { display: 'flex', gap: 10, flexShrink: 0 },
  actionBtnPrimary: { background: '#047857', border: 'none', color: '#fff', borderRadius: 10, padding: '10px 22px', fontSize: 13, fontWeight: 700, cursor: 'pointer' },
  actionBtnDanger: { background: '#fff', border: '1px solid #fecaca', color: '#dc2626', borderRadius: 10, padding: '10px 22px', fontSize: 13, fontWeight: 700, cursor: 'pointer' },

  cancelledBanner: { background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 12, padding: '14px 20px', marginBottom: 20 },
  cancelledBannerTitle: { fontSize: 13, fontWeight: 700, color: '#dc2626', marginBottom: 4 },
  ...
  contactBtn: { display: 'inline-flex', alignItems: 'center', gap: 8, background: '#047857', color: '#fff', borderRadius: 10, padding: '10px 20px', fontSize: 13, fontWeight: 700, textDecoration: 'none' },
  contactBtnOutline: { background: '#fff', color: '#047857', border: '1px solid #a7f3d0' },

  modalBackdrop: { position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(15,23,42,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 },
  modal: { background: '#fff', borderRadius: 20, padding: '36px 32px', maxWidth: 440, width: '100%', textAlign: 'center' as const, boxShadow: '0 24px 60px rgba(0,0,0,0.2)' },
  modalIcon: { fontSize: 32, marginBottom: 14 },
  modalTitle: { fontSize: 22, fontWeight: 800, color: '#0f172a', margin: '0 0 8px' },
  modalSub: { fontSize: 13, color: '#64748b', margin: '0 0 20px', lineHeight: 1.6 },
  cancelTextarea: { width: '100%', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '12px 14px', fontSize: 13, fontFamily: 'inherit', resize: 'vertical' as const, outline: 'none', boxSizing: 'border-box' as const },
  charCount: { fontSize: 11, color: '#94a3b8', textAlign: 'right' as const, marginTop: 4, marginBottom: 12 },
  cancelErrorMsg: { background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#dc2626', fontWeight: 600, marginBottom: 12 },
  modalActions: { display: 'flex', gap: 10, justifyContent: 'center', marginTop: 8 },
  modalBtnBack: { background: '#fff', border: '1px solid #e2e8f0', color: '#64748b', borderRadius: 10, padding: '10px 22px', fontSize: 13, fontWeight: 600, cursor: 'pointer' },
  modalBtnConfirm: { background: '#dc2626', border: 'none', color: '#fff', borderRadius: 10, padding: '10px 22px', fontSize: 13, fontWeight: 700, cursor: 'pointer' },
