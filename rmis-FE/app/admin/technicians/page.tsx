    ...
    <>
      <div className="page-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <p>Review, approve, or reject technician registration applications.</p>
          </div>
      ...
      <div className="summary-cards-container">
        <div className="summary-card">
          ...
          <div className="summary-info">
            <h3>Pending</h3>
            <div className="summary-value">{activeTab === 'PENDING' ? technicians.length : '—'}</div>
          </div>
        </div>
        <div className="summary-card highlight">
          ...
          <div className="summary-info">
            <h3>Active</h3>
            <div className="summary-value">{activeTab === 'ACTIVE' ? technicians.length : '—'}</div>
          </div>
        </div>
        <div className="summary-card">
          <div className="summary-icon" style={{ backgroundColor: '#f0c9c9', color: '#a33b3b' }}>
            <span className="material-symbols-outlined">person_off</span>
          </div>
          <div className="summary-info">
            <h3>Rejected</h3>
            <div className="summary-value">{activeTab === 'REJECTED' ? technicians.length : '—'}</div>
          </div>
        </div>
      </div>
      ...
      <div className="master-table-card" style={{ marginTop: '24px' }}>
        <div className="filters-section" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button
              ...
              style={{
                borderRadius: '8px',
                padding: '8px 16px',
                backgroundColor: userCategory === 'TECHNICIANS' ? '#1a4a38' : 'white',
                color: userCategory === 'TECHNICIANS' ? 'white' : '#4b5563',
                border: userCategory === 'TECHNICIANS' ? 'none' : '1px solid #e5e7eb',
                fontWeight: 600
              }}
            >
              Technicians
            ...
            <button
              ...
              style={{
                borderRadius: '8px',
                padding: '8px 16px',
                backgroundColor: userCategory === 'COMPANIES' ? '#1a4a38' : 'white',
                color: userCategory === 'COMPANIES' ? 'white' : '#4b5563',
                border: userCategory === 'COMPANIES' ? 'none' : '1px solid #e5e7eb',
                fontWeight: 600
              }}
            >
              Company Users
          ...
          <div className="filters-grid" style={{ margin: 0 }}>
            <div className="filter-group" style={{ flexDirection: 'row', alignItems: 'center', gap: '12px' }}>
              <label style={{ whiteSpace: 'nowrap', marginBottom: 0 }}>Filter by Status:</label>
        ...
        <div className="table-section">
          <table className="data-table">
            <thead>
              <tr>
                <th>{userCategory === 'TECHNICIANS' ? 'Technician' : 'Company Name'}</th>
                <th>{userCategory === 'TECHNICIANS' ? 'Specialization' : 'Reg. Number'}</th>
                <th>{userCategory === 'TECHNICIANS' ? 'Experience' : 'Contact Email'}</th>
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px' }}>Loading technicians...</td></tr>
              ) : technicians.length === 0 ? (
                <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px' }}>No users found.</td></tr>
              ) : technicians.map(t => (
                <tr key={t.id} onClick={() => setSelectedTechnician(t)}>
                  <td>
                    <div className="req-id">{userCategory === 'TECHNICIANS' ? `${t.firstName} ${t.lastName}` : (t as any).companyName || t.email}</div>
                    <div style={{ fontSize: '11px', opacity: 0.7 }}>{t.email}</div>
                  </td>
                  <td>{userCategory === 'TECHNICIANS' ? (t.specialization || '—') : (t as any).registrationNumber || '—'}</td>
                  <td>{userCategory === 'TECHNICIANS' ? `${t.yearsOfExperience || 0} yrs` : t.email}</td>
                  <td>{formatDate(t.registrationDate)}</td>

                  <td>
                    <span className={`status-badge ${t.status === 'ACTIVE' ? 'status-approved' :
          ...
          <div className="table-footer" style={{ padding: '16px 32px' }}>
            <div className="pagination-info" style={{ marginLeft: '8px' }}>Count: {technicians.length}</div>
          </div>
      ...
      {showRejectModal && (
        <div style={{
          ...
          <div className="master-table-card" style={{ width: '400px', background: 'white', padding: '24px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              <button onClick={() => setShowRejectModal(false)} className="page-btn">Cancel</button>
              <button onClick={handleRejectSubmit} className="btn-primary" style={{ backgroundColor: '#dc2626' }}>Submit Rejection</button>
            </div>
          </div>
        </div>
      )}
