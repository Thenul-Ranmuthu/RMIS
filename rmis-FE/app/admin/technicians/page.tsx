...
export default function AdminTechnicianPage() {
  ...
  const fetchTechnicians = async () => {
    ...
    try {
      const token = getToken();
      const res = await fetch(`${API_BASE}/admin/technicians/${activeTab.toLowerCase()}`, {
    ...
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f8fafc' }}>
      ...
      <main style={{ flex: 1, padding: '32px', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 28 }}>
          {[
            { label: 'Pending Review', icon: '⏳', bg: '#fffbeb', border: '#fde68a', iconBg: '#fef3c7', count: activeTab === 'PENDING' ? technicians.length : '—', color: '#d97706' },
            { label: 'Active',         icon: '✅', bg: '#f0fdf4', border: '#bbf7d0', iconBg: '#dcfce7', count: activeTab === 'ACTIVE'  ? technicians.length : '—', color: '#16a34a' },
            { label: 'Rejected',       icon: '❌', bg: '#fff1f2', border: '#fecdd3', iconBg: '#fee2e2', count: activeTab === 'REJECTED'? technicians.length : '—', color: '#dc2626' },
          ].map(card => (
            <div key={card.label} style={{
              ...
              <div>
                <p style={{ fontSize: 11, fontWeight: 700, color: card.color, textTransform: 'uppercase', letterSpacing: '0.05em', margin: 0 }}>
                  {card.label}
                </p>
                ...
                <tbody>
                  {technicians.map(t => (
                    <tr
                      ...
                      onMouseEnter={e => { if (selectedTechnician?.id !== t.id) e.currentTarget.style.backgroundColor = '#f8fafc'; }}
                      onMouseLeave={e => { if (selectedTechnician?.id !== t.id) e.currentTarget.style.backgroundColor = 'transparent'; }}
                    >
                      <td style={{ padding: '13px 16px', fontWeight: 600, fontSize: 14, color: '#0f172a' }}>
                      ...
                      <td style={{ padding: '13px 16px' }}>
                        <span style={{ color: '#1a4a38', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                          View →
                        </span>
                      </td>
                    </tr>
            ...
            <div style={{
              ...
              {selectedTechnician.certifications?.length > 0 && (
                <div style={{ marginBottom: 20 }}>
                  <p style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.04em', margin: '0 0 8px' }}>
