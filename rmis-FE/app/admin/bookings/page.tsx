...
const STATUS_META: Record<string, { bg: string; color: string; dot: string; label: string }> = {
  ...
  ACCEPTED:  { bg: '#dbeafe', color: '#1e40af', dot: '#3b82f6', label: 'Accepted'  },
...
const StatusBadge = ({ status }: { status: string }) => {
  const meta = STATUS_META[status] ?? { bg: '#f1f5f9', color: '#475569', dot: '#94a3b8', label: status };
  return (
    <span style={{
    ...
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#f8fafc' }}>
      ...
      <main style={{ flex: 1, padding: '32px', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 28 }}>
          {[
            { label: 'Total',     icon: '📋', bg: '#fff',     border: '#e2e8f0', iconBg: '#f1f5f9', count: tickets.length,            color: '#0f172a' },
            { label: 'Pending',   icon: '⏳', bg: '#fffbeb',  border: '#fde68a', iconBg: '#fef3c7', count: countByStatus('PENDING'),   color: '#d97706' },
            { label: 'Completed', icon: '✅', bg: '#f0fdf4',  border: '#bbf7d0', iconBg: '#dcfce7', count: countByStatus('COMPLETED'), color: '#16a34a' },
            { label: 'Cancelled', icon: '❌', bg: '#fff1f2',  border: '#fecdd3', iconBg: '#fee2e2', count: countByStatus('CANCELLED'), color: '#dc2626' },
          ].map(card => (
            <div key={card.label} style={{
              ...
              <div>
                <p style={{ fontSize: 11, fontWeight: 700, color: card.color === '#0f172a' ? '#94a3b8' : card.color, textTransform: 'uppercase', letterSpacing: '0.05em', margin: 0 }}>
                  {card.label}
                </p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, gap: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{
            ...
            {(['ALL', 'PENDING', 'ACCEPTED', 'COMPLETED', 'CANCELLED'] as StatusFilter[]).map(s => (
              <button
                key={s}
                onClick={() => { setStatusFilter(s); setSelectedTicket(null); }}
                style={{
                  padding: '7px 16px', borderRadius: 7, border: 'none', cursor: 'pointer',
                  fontWeight: 700, fontSize: 12, transition: 'all 0.15s',
                  backgroundColor: statusFilter === s ? '#0f172a' : 'transparent',
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
          ...
          <div style={{
            ...
            {isLoading ? (
              ...
              <div style={{ padding: 48, textAlign: 'center', color: '#94a3b8' }}>
                <div style={{ fontSize: 32, marginBottom: 8 }}>📭</div>
                {searchQuery ? 'No bookings match your search.' : `No ${statusFilter === 'ALL' ? '' : statusFilter.toLowerCase() + ' '}bookings found.`}
              </div>
            ) : (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                      {['Ticket #', 'Customer', 'Technician', 'Service', 'Scheduled', 'Status', ''].map(h => (
                        <th key={h} style={{
                    ...
                    {filtered.map(ticket => (
                      <tr
                        ...
                        style={{
                          ...
                          backgroundColor: selectedTicket?.id === ticket.id ? '#f0fdf4' : 'transparent',
                          transition: 'background-color 0.15s',
                        }}
                        onMouseEnter={e => { if (selectedTicket?.id !== ticket.id) e.currentTarget.style.backgroundColor = '#f8fafc'; }}
                        onMouseLeave={e => { if (selectedTicket?.id !== ticket.id) e.currentTarget.style.backgroundColor = 'transparent'; }}
                      >
                        ...
                        <td style={{ padding: '13px 16px', fontSize: 13, color: '#475569', maxWidth: 160 }}>
                          <p style={{ margin: 0, fontWeight: 600, color: '#0f172a' }}>{ticket.serviceType || '—'}</p>
                          {ticket.description && (
                            <p style={{ margin: '2px 0 0', fontSize: 11, color: '#94a3b8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 140 }}>
                              {ticket.description}
                            </p>
                          )}
                        </td>
                        <td style={{ padding: '13px 16px', whiteSpace: 'nowrap' }}>
                          ...
                          <p style={{ fontSize: 11, color: '#94a3b8', margin: '2px 0 0' }}>
                            {formatTime(ticket.scheduledStartTime)} – {formatTime(ticket.scheduledEndTime)}
                          </p>
                        </td>
                        <td style={{ padding: '13px 16px' }}>
                        ...
                        <td style={{ padding: '13px 16px' }}>
                          <span style={{ color: '#1a4a38', fontSize: 13, fontWeight: 700 }}>View →</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            {!isLoading && filtered.length > 0 && (
              <div style={{
                ...
                <span>Showing {filtered.length} of {tickets.length} bookings</span>
          ...
          {selectedTicket && (
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                <div>
                  <h3 style={{ fontSize: 15, fontWeight: 800, color: '#0f172a', margin: 0, fontFamily: 'monospace' }}>
                    {selectedTicket.ticketNumber}
                  </h3>
                  <p style={{ fontSize: 12, color: '#64748b', margin: '3px 0 0' }}>Ticket ID #{selectedTicket.id}</p>
                </div>
                <button
              ...
              {[
                {
                  title: '👤 Customer',
                  rows: [
                    { label: 'Name',  value: selectedTicket.customerName  || '—' },
                    { label: 'Email', value: selectedTicket.customerEmail || '—' },
                  ],
                },
                {
                  title: '🔧 Technician',
                  rows: [
                    { label: 'Name',           value: selectedTicket.technicianName           || '—' },
                    { label: 'Specialization', value: selectedTicket.technicianSpecialization || '—' },
                  ],
                },
                {
                  title: '📅 Schedule',
                  rows: [
                    { label: 'Date',       value: formatDate(selectedTicket.scheduledDate) },
                    { label: 'Start Time', value: formatTime(selectedTicket.scheduledStartTime) },
                    { label: 'End Time',   value: formatTime(selectedTicket.scheduledEndTime) },
                  ],
                },
                {
                  title: '🛠️ Service',
                ...
                },
                {
                  ...
                  rows: [
                    ...
                    { label: 'Updated', value: formatDateTime(selectedTicket.updatedAt) },
                  ],
                },
              ].map(section => (
              ...
              {selectedTicket.status === 'CANCELLED' && (
                <div style={{
                  ...
                  <p style={{ fontSize: 11, fontWeight: 700, color: '#dc2626', textTransform: 'uppercase', letterSpacing: '0.04em', margin: '0 0 4px' }}>
